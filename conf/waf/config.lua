RulePath = "/usr/local/openresty/nginx/conf/waf/wafconf/" --规则存放目录
attacklog = "on" --是否开启攻击信息记录，需要配置logdir
logdir = "/data/log/" --log存储目录，该目录需要用户自己新建，切需要nginx用户的可写权限
...
CCDeny="off" --是否开启拦截cc攻击(需要nginx.conf的http段增加lua_shared_dict limit 10m;)
