...
topic_router:get("/:topic_id/query", function(req, res, next)
    local topic_id = req.params.topic_id
    local current_userid = 0; --当前用户id
    
    if not topic_id then
        return res:json({success = false,msg = "要查询的文章id参数不能为空."})
    end 
     
    if req.me and req.me.userid then
        current_userid = tonumber(req.me.userid);
    end
    ...
    if not result or err or type(result) ~= "table" or #result ~= 1 then
        return res:json({ succes= false, msg = "无法查找到该文章."})
    else
...
topic_router:get("/:topic_id/edit", function(req, res, next)
    ...
    local current_userid = req.me.userid
